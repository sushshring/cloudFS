...
class Cloudfs {
 public:
   ...
   * Introduced in version 2.8
   */
  int poll(const std::string& path, const std::shared_ptr<fuse_file_info> file_info,
   ...
   */
  int write_buf(const std::string& path,
                std::shared_ptr<fuse_bufvec> buf, off_t off,
                std::shared_ptr<fuse_file_info> file_info);
   ...
   * Introduced in version 2.9
   */
  int read_buf(const std::string& path,
               const std::shared_ptr<fuse_bufvec> bufp, size_t size,
  ...
  static std::unique_ptr<Cloudfs>& instance(
